...
app.get('/page/resRecv', function(req, res){
    res.sendFile(__dirname + '/viewpage/resRecv.html');
})
...
app.post('/api/createReceivable', async function(req, res){
    ...
    var args = [recvnumber, reptnumber, ownername, havedlist, dealprice, issuerate, publishdate, expiredate, isguarantee, issale]
...
app.post('/api/queryReqRecv', async function(req,res){
    const recvno = req.body.recvno
    const result = await callChainCode('queryReqRecv', recvno)
    res.json(JSON.parse(result))
})
app.post('/api/acceptReqPurchase', async function(req, res){
    const reqno = req.body.reqno
    const recvno = req.body.recvno
    const reqorg = req.body.reqorg
    const dealprice = req.body.dealprice
    const reqarr = JSON.stringify(req.body.reqArray)
    console.log(reqno+' '+recvno+' '+reqorg+' '+dealprice+' '+reqarr)
    var args = [reqno, recvno, reqorg, dealprice, reqarr]
    await callChainCode('acceptReqPurchase', args)
    res.status(200).json({result: "success"})
})
...
async function callChainCode(fnName, args){
    ...
    if(fnName == 'createSellItem')
        ...
        result = await contract.submitTransaction(fnName,args);
    else if(fnName == 'queryReqRecv')
        result = await contract.submitTransaction(fnName,args);
    else if(fnName == 'acceptReqPurchase')
