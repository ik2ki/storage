...
app.get('/page/queryAllReqPurchases', function(req,res){
    res.sendFile(__dirname + '/viewpage/queryAllReqPurchases.html');
})
...
app.get('/api/queryReqPurchases', async function(req, res){
    const result = await callChainCode('queryReqPurchases')
    res.json(JSON.parse(result))
})
...
async function callChainCode(fnName, args){
    ...
    if(fnName == 'queryAllItems')
        ...
        result = await contract.evaluateTransaction(fnName);
    else if(fnName == 'queryReqPurchases')
        result = await contract.evaluateTransaction(fnName);
